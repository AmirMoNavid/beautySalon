...
export const getAllSalons = async (req, res) => {
  ...
  if (!isNaN(count)) {
    const gallerysCount = await Salon.count();
    return res.send(gallerysCount.toString());
  }

  try {
    const gallerys = await Salon.findAll({
...
export const updateSalon = async (req, res) => {
  ...
  if (!salon) return res.status(400).json({ title: "دیتایی وجود ندارد." });

  let fileName = "";
  if (req.files === null) {
    fileName = salon.image;
  } else {
    const file = req.files?.file;
    const fileSize = file?.data?.length;
    const ext = path.extname(file?.name);
    ...
    const filePath = path.join(paths.publicDir, "images", salon.image);
